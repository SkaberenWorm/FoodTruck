...
class WC_POS_Gateways_Cash extends WC_Payment_Gateway {
  ...
  public function payment_fields() {
    ...
    echo '
      <div class="form-row" id="pos-cash-tendered_field">
        ...
        '. $left_addon .'
          <input type="text" class="form-control" name="pos-cash-tendered" id="pos-cash-tendered" maxlength="20"  data-label="'. __('Amount Tendered', 'woocommerce-pos') .'" data-placement="bottom" data-value="{{total}}">
        '. $right_addon .'
        </div>
      </div>
    ';
  ...
  static public function payment_details( $payment_details, $order ) {
    $id = is_integer($order) ? $order : $order->id;

    $payment_details['method_pos_cash'] = array(
      'tendered'  => get_post_meta( $id, '_pos_cash_amount_tendered', true ),
      'change'    => get_post_meta( $id, '_pos_cash_change', true )
    );
